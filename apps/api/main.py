...
from packages.config.settings import load_settings, SpectreSettings
...
async def workflow_history(limit: int = 50) -> list[dict[str, Any]]:
...
async def workflow_definitions() -> dict[str, Any]:
...
async def load_workflows(directory: str) -> dict[str, Any]:
...
async def kernel_start() -> dict[str, str]:
    """Start the Kernel."""
    k = _get_kernel()
    if k.running:
...
async def kernel_stop() -> dict[str, str]:
    """Stop the Kernel."""
    k = _get_kernel()
    if not k.running:
...
async def service_bus_status() -> dict[str, Any]:
...
@app.get("/api/reports")
async def list_reports(report_type: str | None = None, limit: int = 50) -> list[dict[str, Any]]:
...
async def get_config(key: str, profile: str = "default") -> dict[str, str]:
...
async def set_config(key: str, value: str, profile: str = "default") -> dict[str, str]:
